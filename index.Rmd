...
cln_inc <- cln_inc %>% slice(seq(1, n(), by = 5))
cln_mort <-  cln_mort %>% slice(seq(1, n(), by = 5))
```
...
```{r, include=TRUE,echo=FALSE, warning=FALSE, message = FALSE}
cln_mort <- subset(cln_mort, cln_mort$CRUDE_RATE < 50)
...
cln_inc %>%
  plot_ly(x = ~YEAR, y = ~CRUDE_RATE,
          ...
          colors = "Blues",
          size = ~CRUDE_RATE) %>%
  layout(title="FIGURE 1. Interactive Overall U.S. Colorectal Cancer INCIDENCE rate (1999 to 2017)")
...
cln_mort %>%
  plot_ly(x = ~YEAR, y = ~CRUDE_RATE,
          ...
          colors = "Reds",
          size = ~CRUDE_RATE) %>%
  layout(title="FIGURE 2. Interactive Overall U.S. Colorectal Cancer MORTALITY rate (1999 to 2017)")
...
df <- cln_mort %>% select(code, POPULATION, CRUDE_RATE, AGE_ADJUSTED_RATE) # select data
...
#removing outlier
cln4 <- subset(cln3, cln3$AGE_ADJUSTED_RATE < 100)
